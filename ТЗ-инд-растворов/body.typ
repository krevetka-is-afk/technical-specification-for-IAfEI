...
#let column_names = (
    ...
    [Универсальная генеративная модель без корпуса],
)
...
#figure(
    ...
    table(
        ...
        [Локальное развертывание], plus, minus, minus,
        [Серверная изоляция памяти и защита границ доверия], plus, minus, minus,
...
Стадии и этапы разработки определены с учетом ГОСТ 19.102-77 [2].
...
#figure(
    ...
    text(size: 11pt, table(
        ...
        [Проработка предметной области и требований], [Определение источников данных и пользовательских сценариев], [1.11.25 --\ 10.12.25],
        [Согласование ТЗ], [Утверждение состава серверных функций и технических метрик], [10.12.25 --\ 16.12.25],
        ...
        [Подготовка данных], [Загрузка документов PDF, HTML, HTM, TXT и DOCX, нормализация текста и формирование метаданных], [01.01.26 --\ 10.03.26],
        [Векторное хранилище], [Построение и валидация индекса Chroma на корпусе документов], [01.01.26 --\ 24.02.26],
        [Генеративная модель и правила обращения], [Настройка Ollama, системных инструкций, ограничений контекста и резервного ответа], [01.01.26 --\ 24.02.26],
        [Серверные интерфейсы], [Интеграция Telegram-бота и веб-интерфейса с серверным API, авторизация, память и журналирование], [25.02.26 --\ 25.03.26],
        ...
        [Функциональные и нагрузочные испытания], [Прогон сценариев, замер времени ответа и устойчивости], [25.02.26 --\ 15.03.26],
        [Пилотная группа], [Пилотное тестирование, сбор и анализ обратной связи], [01.04.26 --\ 16.04.26],
        [Документация и сдача], [Актуализация серверных инструкций, развертывание на тестовом сервере, отчетность], [16.04.26 --\ 25.04.26],
    ))
)
...
- Сбор, очистка и структурирование данных университета — А. К. Соколова.
- Разработка и настройка серверного ядра системы: векторная база данных, метод RAG, генеративная модель, программный API, память и метрики — С. Е. Растворов.
- Создание пользовательских интерфейсов и организация тестирования с реальными пользователями — Л. А. Субботин.
